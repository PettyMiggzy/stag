 ...
 *   • GAMBLE — flat 0.010; a rarity-WEIGHTED random draw picks one. Top tiers are rare to hit.
 *              Both modes share ONE pool → no duplicates ever.
 *
 *  WHY THEY'RE WORTH SOMETHING — STAKING UTILITY:
 *   These lock-in-place stake into StagStaking exactly like The Hooded 20 (the staking contract is the
 *   "locker"). Rarer tokens carry a bigger reward boost: boostBpsOf(id) maps each tier → a suggested
 *   nftBoostBps the owner wires into StagStaking.setNftBoostBps(id, bps). A staked Mythic earns far more
 *   real-yield ETH than a Common. That utility — not just the art — is the floor.
 *
...
contract KidWaltShow is ERC721Enumerable, ERC2981, Ownable, ReentrancyGuard {
    uint256 public constant MAX_SUPPLY = 10;

    // Tier of each token id (1..10), one byte per id. 0=Common 1=Rare 2=Epic 3=Legendary 4=Mythic.
    //  1 kid_host_desk=Common  2 kid_point_mic=Common  3 kid_present_coin=Rare  4 kid_archer=Epic
    //  5 kid_welcome=Legendary 6 walt_cohost=Common     7 walt_flex=Rare        8 walt_point=Epic
    //  9 walt_present_coin=Rare 10 walt_laugh=Mythic
    bytes public constant TIERS = hex"00000102030001020104"; // id1..id10
    ...
    // The NFT does not enforce this; the owner wires it via StagStaking.setNftBoostBps(id, boostBpsOf(id)).
    uint256[5] public tierBoostBps = [uint256(1000), 2000, 3000, 4000, 5000];
    uint256    public maxPerWallet = 3;
    bool       public mintActive;

    string  private _base;                 // e.g. https://stagwifhood.fun/assets/nft/kidwalt/metadata/
        ...
        Ownable(msg.sender)
    {
        ...
        // seed the pool with ids 1..10 and accumulate their weights
    ...
    // suggested StagStaking reward boost (bps) for this token, by its rarity tier.
    ...
    function _mintOne(uint256 id, uint256 price, bool gamble) internal {
        ...
        // fan-out inside the user's signed mint tx. The 90/10 split is done out-of-band via
        // forwardProceeds() so a single-recipient mint stays scanner-clean (no drainer flag).
    }

    // Batch-forward accumulated mint proceeds to the splitter (90% pool / 10% owner). Permissionless
    // and NOT part of any user-signed mint — run it from a keeper/cron/admin.
    ...
    // For a 10-piece art mint this is an accepted trade-off; tiered PICK pricing bounds the
    // incentive to snipe rares via GAMBLE.
