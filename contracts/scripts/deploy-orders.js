...
async function main() {
  [ROUTER, WETH, FACTORY, FEE_WALLET, NEW_OWNER].forEach((a) => ethers.getAddress(a));
  const [d] = await ethers.getSigners();
  console.log("deployer:", d.address);
  const orders = await (await ethers.getContractFactory("SherwoodOrders")).deploy(ROUTER, WETH, FACTORY, FEE_WALLET);
  ...
  console.log("Verify:  npx hardhat verify --network rhmainnet " + addr + " " + ROUTER + " " + WETH + " " + FACTORY + " " + FEE_WALLET);
}
